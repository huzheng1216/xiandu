...
package com.inveno.xiandu.view.components.tablayout;
        ...
        public void draw(Canvas canvas) {
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                ...
                if (mSelectedIndicatorWidth > 0 && getSelectedTabPosition() >= 0 &&
                    ...
                    if (mViewPager == null || !mIsDragged || firstDraw) {
                        firstDraw = false;
...
//                    LogTools.H("&&&"+left+"-"+(getHeight() - mSelectedIndicatorHeight)+"-"+right+"-"+getHeight());
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            if (myTabLayout != null) {
                ...
                if (myTabLayout.mNeedSwitchAnimation) {
                    if (mScrollState == SCROLL_STATE_SETTLING && mPreviousScrollState == SCROLL_STATE_IDLE) {
                        myTabLayout.mTabStrip.mIsDragged = false;
                    } else {
                        myTabLayout.mTabStrip.mIsDragged = true;
                        if (myTabLayout.mTabStrip.mSelectedPosition + 1 < myTabLayout.mTabStrip.getChildCount()) {
                            float offset = (myTabLayout.mTabStrip.getChildAt(myTabLayout.mTabStrip.mSelectedPosition).getMeasuredWidth()
                                    - myTabLayout.mTabStrip.getWrapContentIndicatorWidth(myTabLayout.mTabStrip.mSelectedPosition)
                            ) / 2.0f;
                            float leftX = myTabLayout.mTabStrip.getChildAt(myTabLayout.mTabStrip.mSelectedPosition).getLeft() + offset;
                            float rightX = leftX + myTabLayout.mTabStrip.getWrapContentIndicatorWidth(myTabLayout.mTabStrip.mSelectedPosition);

                            float nextOffset = (myTabLayout.mTabStrip.getChildAt(myTabLayout.mTabStrip.mSelectedPosition + 1).getMeasuredWidth()
                                    - myTabLayout.mTabStrip.getWrapContentIndicatorWidth(myTabLayout.mTabStrip.mSelectedPosition + 1)
                            ) / 2.0f;
                            float nextLeftX = myTabLayout.mTabStrip.getChildAt(myTabLayout.mTabStrip.mSelectedPosition).getLeft()
                                    + myTabLayout.mTabStrip.getChildAt(myTabLayout.mTabStrip.mSelectedPosition).getWidth() + nextOffset;
                            float nextRightX = nextLeftX +
                                    myTabLayout.mTabStrip.getWrapContentIndicatorWidth(myTabLayout.mTabStrip.mSelectedPosition + 1);
                            myTabLayout.mTabStrip.left = leftX + (nextLeftX - leftX) * mStartInterpolator.getInterpolation(positionOffset);
                            myTabLayout.mTabStrip.right = rightX + (nextRightX - rightX) * mEndInterpolator.getInterpolation(positionOffset);
                            ViewCompat.postInvalidateOnAnimation(myTabLayout.mTabStrip);
                        }
                    }
                }
            }
